...
const UINT MSWindowsClientTaskBarReceiver::s_stateToIconID[kMaxState] = {IDI_TASKBAR_NOT_RUNNING,
                                                                         IDI_TASKBAR_NOT_WORKING,
                                                                         IDI_TASKBAR_NOT_CONNECTED,
                                                                         IDI_TASKBAR_NOT_CONNECTED,
                                                                         IDI_TASKBAR_CONNECTED};

MSWindowsClientTaskBarReceiver::MSWindowsClientTaskBarReceiver(HINSTANCE appInstance, const BufferedLogOutputter *logBuffer, IEventQueue *events)
...
void MSWindowsClientTaskBarReceiver::runMenu(int x, int y)
{
    ...
    int n = TrackPopupMenu(menu, TPM_NONOTIFY | TPM_RETURNCMD | TPM_LEFTBUTTON | TPM_RIGHTBUTTON, x, y, 0, m_window, NULL);
...
{
    ArchMiscWindows::setIcons((HICON)LoadImage(ArchMiscWindows::instanceWin32(), MAKEINTRESOURCE(IDI_DESKFLOW), IMAGE_ICON, 32, 32, LR_SHARED),
