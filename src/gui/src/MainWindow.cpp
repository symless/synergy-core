...
void MainWindow::connectSlots()
{
    ...
    connect(&m_ServerConnection, &ServerConnection::configureClient, this, &MainWindow::onServerConnectionConfigureClient);

    connect(&m_ServerConnection, &ServerConnection::messageShowing, this, [this]() {
        showAndActivate();
    });

    connect(&m_ClientConnection, &ClientConnection::messageShowing, this, [this]() {
        showAndActivate();
    });
}
...
void MainWindow::onShown()
{
    ...
    QTimer::singleShot(kCriticalDialogDelay, [] {
        messages::raiseCriticalDialog();
    });
}
...
void MainWindow::onCoreProcessError(CoreProcess::Error error)
{
    if (error == CoreProcess::Error::AddressMissing) {
        QMessageBox::warning(this, QString("Address missing"), QString("Please enter the hostname or IP address of the other computer."));
    } else if (error == CoreProcess::Error::StartFailed) {
        show();
        QMessageBox::warning(this,
                             QString("Core cannot be started"),
...
{
    std::vector<QAction *> trayMenu = {m_pActionStartCore, m_pActionStopCore, nullptr, m_pActionMinimize, m_pActionRestore, nullptr, m_pActionQuit};
...
void MainWindow::checkFingerprint(const QString &line)
{
    ...
    if (!messageBoxAlreadyShown) {
        ...
        QMessageBox::StandardButton fingerprintReply = QMessageBox::information(this,
                                                                                QString("Security question"),
...
void MainWindow::onCoreProcessStateChanged(CoreProcessState state)
{
    ...
    if (state == CoreProcessState::Started || state == CoreProcessState::Starting || state == CoreProcessState::RetryPending) {
