...
// names of priorities
static const char *g_priority[] = {"FATAL", "ERROR", "WARNING", "NOTE", "INFO", "DEBUG", "DEBUG1", "DEBUG2", "DEBUG3", "DEBUG4", "DEBUG5"};
...
std::vector<char> makeMessage(const char *filename, int lineNumber, const char *message, ELevel priority)
{
    ...
    if (filenameSet) {
        ...
        snprintf(buffer.data(), bufferSize, "[%s] %s: %s\n\t%s:%d", timeBuffer.data(), g_priority[priority], message, filename, lineNumber);
        return buffer;
    } else {
        std::vector<char> buffer(bufferSize);
