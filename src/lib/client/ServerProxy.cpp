    ...
    , m_events(events)
{
    ...
    m_events->adoptHandler(m_events->forIStream().inputReady(), m_stream->getEventTarget(), new TMethodEventJob<ServerProxy>(this, &ServerProxy::handleData));
...
void ServerProxy::resetKeepAliveAlarm()
{
    ...
    if (m_keepAliveAlarm > 0.0) {
        ...
        m_events->adoptHandler(Event::kTimer, m_keepAliveAlarmTimer, new TMethodEventJob<ServerProxy>(this, &ServerProxy::handleKeepAliveAlarm));
    }
}
...
KeyModifierMask ServerProxy::translateModifierMask(KeyModifierMask mask) const
{
    static const KeyModifierMask s_masks[kKeyModifierIDLast] =
        {0x0000, KeyModifierShift, KeyModifierControl, KeyModifierAlt, KeyModifierMeta, KeyModifierSuper, KeyModifierAltGr};

    KeyModifierMask newMask = mask & ~(KeyModifierShift | KeyModifierControl | KeyModifierAlt | KeyModifierMeta | KeyModifierSuper | KeyModifierAltGr);
...
void ServerProxy::checkMissedLanguages() const
{
    ...
    if (!missedLanguages.empty()) {
        LOG((CLOG_WARN "You need to install these languages on this computer and restart "
