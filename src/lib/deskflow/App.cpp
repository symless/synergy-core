...
void App::loggingFilterWarning()
{
    if (CLOG->getFilter() > CLOG->getConsoleMaxLevel()) {
        if (argsBase().m_logFile == NULL) {
            LOG((CLOG_WARN "log messages above %s are NOT sent to console (use file logging)", CLOG->getFilterName(CLOG->getConsoleMaxLevel())));
        }
    }
}
...
void App::initApp(int argc, const char **argv)
{
    ...
    if (!CLOG->setFilter(argsBase().m_logFilter)) {
        LOG((CLOG_CRIT "%s: unrecognized log level `%s'" BYE, argsBase().m_pname, argsBase().m_logFilter, argsBase().m_pname));
        m_bye(kExitArgs);
    }
...
void App::initIpcClient()
{
    ...
    m_events->adoptHandler(m_events->forIpcClient().messageReceived(), m_ipcClient, new TMethodEventJob<App>(this, &App::handleIpcMessage));
}
...
MinimalApp::MinimalApp()
    : App(NULL, NULL, new deskflow::ArgsBase())
{
    m_arch.init();
    setEvents(m_events);
}
