...
deskflow::Screen *ClientApp::createScreen()
{
    ...
    return new deskflow::Screen(
        new XWindowsScreen(args().m_display, false, args().m_disableXInitThreads, args().m_yscroll, m_events, args().m_clientScrollDirection),
        m_events);

#endif

#if WINAPI_CARBON
    return new deskflow::Screen(new OSXScreen(m_events, false, args().m_enableLangSync, args().m_clientScrollDirection), m_events);
#endif
}
...
deskflow::Screen *ClientApp::openClientScreen()
{
    ...
    m_events->adoptHandler(m_events->forIScreen().error(), screen->getEventTarget(), new TMethodEventJob<ClientApp>(this, &ClientApp::handleScreenError));
    return screen;
}
...
void ClientApp::scheduleClientRestart(double retryTime)
{
    ...
    m_events->adoptHandler(Event::kTimer, timer, new TMethodEventJob<ClientApp>(this, &ClientApp::handleClientRestart, timer));
}
...
Client *ClientApp::openClient(const String &name, const NetworkAddress &address, deskflow::Screen *screen)
{
    ...
    try {
        ...
        m_events->adoptHandler(m_events->forClient().connectionRefused(),
                               client->getEventTarget(),
