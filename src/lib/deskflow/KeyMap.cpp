...
void KeyMap::addKeyAliasEntry(KeyID targetID,
                              SInt32 group,
...
const KeyMap::KeyItem *KeyMap::mapKey(Keystrokes &keys,
                                      KeyID id,
                                      SInt32 group,
...
{
    LOG((CLOG_DEBUG1 "mapKey %04x (%d) with mask %04x, start state: %04x, group: %d", id, id, desiredMask, currentState, group));
    ...
    case kKeyClearModifiers:
        if (!keysForModifierState(0, group, activeModifiers, currentState, currentState & ~desiredMask, desiredMask, 0, keys)) {
...
const KeyMap::KeyItemList *KeyMap::findCompatibleKey(KeyID id, SInt32 group, KeyModifierMask required, KeyModifierMask sensitive) const
{
    ...
    for (size_t j = 0; j < entries.size(); ++j) {
        if ((entries[j].back().m_sensitive & sensitive) == 0 || (entries[j].back().m_required & sensitive) == (required & sensitive)) {
            return &entries[j];
        }
    }
...
const KeyMap::KeyItem *KeyMap::mapCommandKey(Keystrokes &keys,
                                             KeyID id,
                                             SInt32 group,
                                             ModifierToKeys &activeModifiers,
                                             KeyModifierMask &currentState,
                                             ...
                                             const String &lang) const
{
    ...
    // add the key
    if (!keysForKeyItem(*keyItem, newGroup, newModifiers, newState, desiredMask, s_overrideModifiers, isAutoRepeat, keys, lang)) {
...
const KeyMap::KeyItemList *KeyMap::getKeyItemList(const KeyMap::KeyGroupTable &keyGroupTable, SInt32 group, KeyModifierMask desiredMask) const
{
...
const KeyMap::KeyItem *KeyMap::mapCharacterKey(Keystrokes &keys,
                                               KeyID id,
                                               SInt32 group,
                                               ModifierToKeys &activeModifiers,
                                               KeyModifierMask &currentState,
...
const KeyMap::KeyItem *KeyMap::mapModifierKey(Keystrokes &keys,
                                              KeyID id,
                                              SInt32 group,
                                              ModifierToKeys &activeModifiers,
                                              KeyModifierMask &currentState,
...
SInt32 KeyMap::findBestKey(const KeyEntryList &entryList, KeyModifierMask desiredState) const
{
    ...
    for (SInt32 i = 0; i < (SInt32)entryList.size(); ++i) {
        const KeyItem &item = entryList[i].back();
        if ((item.m_required & desiredState) == item.m_required && (item.m_required & desiredState) == (item.m_sensitive & desiredState)) {
...
bool KeyMap::keysForKeyItem(const KeyItem &keyItem,
                            SInt32 &group,
                            ModifierToKeys &activeModifiers,
                            KeyModifierMask &currentState,
                            ...
                            const String &lang) const
{
    ...
    if (keyItem.m_dead) {
        ...
        if (!keysForModifierState(keyItem.m_button, group, activeModifiers, currentState, keyItem.m_required, keyItem.m_sensitive, 0, keystrokes)) {
        ...
        if (!keysForModifierState(keyItem.m_button, group, activeModifiers, currentState, keyItem.m_required, sensitive, 0, keystrokes)) {
...
bool KeyMap::keysToRestoreModifiers(const KeyItem &keyItem,
                                    SInt32,
                                    ModifierToKeys &activeModifiers,
                                    KeyModifierMask &currentState,
...
bool KeyMap::keysForModifierState(KeyButton button,
                                  SInt32 group,
                                  ModifierToKeys &activeModifiers,
                                  KeyModifierMask &currentState,
                                  ...
                                  Keystrokes &keystrokes) const
{
    ...
    while (bit-- > 0) {
        ...
        if (sensitive != 0) {
            ...
            if (active && !keysForModifierState(button, group, activeModifiers, currentState, keyItem->m_required, sensitive, notRequiredMask, keystrokes)) {
                return false;
            } else if (!active) {
...
void KeyMap::addKeystrokes(EKeystroke type,
                           ...
                           Keystrokes &keystrokes) const
{
    ...
    case kKeystrokeRelease:
        ...
        if (keyItem.m_generates != 0 && !keyItem.m_lock) {
            ...
            std::pair<ModifierToKeys::iterator, ModifierToKeys::iterator> range = activeModifiers.equal_range(keyItem.m_generates);
    ...
    case kKeystrokeUnmodify:
        if (keyItem.m_lock) {
            ...
            // currently down
            std::pair<ModifierToKeys::const_iterator, ModifierToKeys::const_iterator> range = activeModifiers.equal_range(keyItem.m_generates);
...
{
    return (m_id == x.m_id && m_group == x.m_group && m_button == x.m_button && m_required == x.m_required && m_sensitive == x.m_sensitive
            && m_generates == x.m_generates && m_dead == x.m_dead && m_lock == x.m_lock && m_client == x.m_client);
}
...
KeyMap::Keystroke::Keystroke(KeyButton button, bool press, bool repeat, UInt32 data)
    : m_type(kButton)
{
