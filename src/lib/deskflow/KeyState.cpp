...
static const KeyID s_numpadTable[] = {
    kKeyKP_Space, 0x0020,    kKeyKP_Tab,       kKeyTab,    kKeyKP_Enter,    kKeyReturn, kKeyKP_F1,       kKeyF1,       kKeyKP_F2,       kKeyF2,
    kKeyKP_F3,    kKeyF3,    kKeyKP_F4,        kKeyF4,     kKeyKP_Home,     kKeyHome,   kKeyKP_Left,     kKeyLeft,     kKeyKP_Up,       kKeyUp,
    kKeyKP_Right, kKeyRight, kKeyKP_Down,      kKeyDown,   kKeyKP_PageUp,   kKeyPageUp, kKeyKP_PageDown, kKeyPageDown, kKeyKP_End,      kKeyEnd,
    kKeyKP_Begin, kKeyBegin, kKeyKP_Insert,    kKeyInsert, kKeyKP_Delete,   kKeyDelete, kKeyKP_Equal,    0x003d,       kKeyKP_Multiply, 0x002a,
    kKeyKP_Add,   0x002b,    kKeyKP_Separator, 0x002c,     kKeyKP_Subtract, 0x002d,     kKeyKP_Decimal,  0x002e,       kKeyKP_Divide,   0x002f,
    kKeyKP_0,     0x0030,    kKeyKP_1,         0x0031,     kKeyKP_2,        0x0032,     kKeyKP_3,        0x0033,       kKeyKP_4,        0x0034,
    kKeyKP_5,     0x0035,    kKeyKP_6,         0x0036,     kKeyKP_7,        0x0037,     kKeyKP_8,        0x0038,       kKeyKP_9,        0x0039};
...
void KeyState::sendKeyEvent(void *target, bool press, bool isAutoRepeat, KeyID key, KeyModifierMask mask, SInt32 count, KeyButton button)
{
...
void KeyState::fakeKeyDown(KeyID id, KeyModifierMask mask, KeyButton serverID, const String &lang)
{
    ...
    const deskflow::KeyMap::KeyItem *keyItem = m_keyMap.mapKey(keys, id, pollActiveGroup(), m_activeModifiers, getActiveModifiersRValue(), mask, false, lang);

    if (keyItem == nullptr) {
        // a media key won't be mapped on mac, so we need to fake it in a
        // special way
        if (id == kKeyAudioDown || id == kKeyAudioUp || id == kKeyAudioMute || id == kKeyAudioPlay || id == kKeyAudioPrev || id == kKeyAudioNext
...
bool KeyState::fakeKeyRepeat(KeyID id, KeyModifierMask mask, SInt32 count, KeyButton serverID, const String &lang)
{
    ...
    Keystrokes keys;
    ModifierToKeys oldActiveModifiers = m_activeModifiers;
    const deskflow::KeyMap::KeyItem *keyItem = m_keyMap.mapKey(keys, id, pollActiveGroup(), m_activeModifiers, getActiveModifiersRValue(), mask, true, lang);
    if (keyItem == NULL) {
        return false;
    }
    KeyButton localID = (KeyButton)(keyItem->m_button & kButtonMask);
...
void KeyState::addAliasEntries()
{
    for (SInt32 g = 0, n = m_keyMap.getNumGroups(); g < n; ++g) {
        ...
        m_keyMap.addKeyAliasEntry(kKeyTab, g, KeyModifierShift, KeyModifierShift, kKeyLeftTab, KeyModifierShift, KeyModifierShift);
...
void KeyState::updateModifierKeyState(KeyButton button, const ModifierToKeys &oldModifiers, const ModifierToKeys &newModifiers)
{
    ...
    std::set_difference(oldKeys.begin(), oldKeys.end(), newKeys.begin(), newKeys.end(), std::inserter(released, released.end()), ButtonToKeyLess());
    std::set_difference(newKeys.begin(), newKeys.end(), oldKeys.begin(), oldKeys.end(), std::inserter(pressed, pressed.end()), ButtonToKeyLess());
...
KeyState::AddActiveModifierContext::AddActiveModifierContext(SInt32 group, KeyModifierMask mask, ModifierToKeys &activeModifiers)
    : m_activeGroup(group)
    , m_mask(mask)
