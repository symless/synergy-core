...
template<typename T>
void writeVectorInt(const std::vector<T> *VectorData, std::vector<UInt8> &Buffer)
{
    if (VectorData) {
...
UInt32 ProtocolUtil::read4BytesInt(deskflow::IStream *stream)
{
    ...
    UInt32 Result = (static_cast<UInt32>(buffer[0]) << 24) | (static_cast<UInt32>(buffer[1]) << 16) | (static_cast<UInt32>(buffer[2]) << 8)
