...
void ServerApp::closeServer(Server *server)
{
    ...
    m_events->adoptHandler(Event::kTimer, timer, new TMethodEventJob<ServerApp>(this, &ServerApp::handleClientsDisconnected));
    m_events->adoptHandler(m_events->forServer().disconnected(), server, new TMethodEventJob<ServerApp>(this, &ServerApp::handleClientsDisconnected));
...
deskflow::Screen *ServerApp::openServerScreen()
{
    ...
    m_events->adoptHandler(m_events->forIScreen().error(), screen->getEventTarget(), new TMethodEventJob<ServerApp>(this, &ServerApp::handleScreenError));
    m_events->adoptHandler(m_events->forIScreen().suspend(), screen->getEventTarget(), new TMethodEventJob<ServerApp>(this, &ServerApp::handleSuspend));
    m_events->adoptHandler(m_events->forIScreen().resume(), screen->getEventTarget(), new TMethodEventJob<ServerApp>(this, &ServerApp::handleResume));
    return screen;
}
...
{
#if WINAPI_MSWINDOWS
    return new deskflow::Screen(new MSWindowsScreen(true, args().m_noHooks, args().m_stopOnDeskSwitch, m_events), m_events);
#endif
    ...
    return new deskflow::Screen(new XWindowsScreen(args().m_display, true, args().m_disableXInitThreads, 0, m_events), m_events);
#elif WINAPI_CARBON
...
ClientListener *ServerApp::openClientListener(const NetworkAddress &address)
{
    ...
    m_events->adoptHandler(m_events->forClientListener().connected(), listen, new TMethodEventJob<ServerApp>(this, &ServerApp::handleClientConnected, listen));
...
Server *ServerApp::openServer(ServerConfig &config, PrimaryClient *primaryClient)
{
    ...
    try {
        m_events->adoptHandler(m_events->forServer().disconnected(), server, new TMethodEventJob<ServerApp>(this, &ServerApp::handleNoClients));

        m_events->adoptHandler(m_events->forServer().screenSwitched(), server, new TMethodEventJob<ServerApp>(this, &ServerApp::handleScreenSwitched));
...
int ServerApp::mainLoop()
{
    ...
    m_events->adoptHandler(m_events->forServerApp().resetServer(), m_events->getSystemTarget(), new TMethodEventJob<ServerApp>(this, &ServerApp::resetServer));
