...
void X11LayoutsParser::convertLayoutToISO639_2(const String &pathToEvdevFile,
                                               ...
                                               std::vector<String> &iso639_2Codes)
{
    ...
    for (size_t i = 0; i < layoutNames.size(); i++) {
        ...
        auto langIter = std::find_if(allLang.begin(), allLang.end(), [&layoutName](const Lang &l) {
        ...
        if (layoutVariantNames[i].empty()) {
            ...
            auto langVariantIter = std::find_if(langIter->variants.begin(), langIter->variants.end(), [&variantName](const Lang &l) {
...
String X11LayoutsParser::convertLayotToISO(const String &pathToEvdevFile, const String &layoutLangCode, bool needToReloadFiles)
{
    std::vector<String> iso639_2Codes;
...
std::vector<String> X11LayoutsParser::convertISO639_2ToISO639_1(const std::vector<String> &iso639_2Codes)
{
    ...
    for (const auto &isoCode : iso639_2Codes) {
        const auto &tableIter = std::find_if(ISO_Table.begin(), ISO_Table.end(), [&isoCode](const std::pair<String, String> &c) {
