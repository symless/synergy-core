...
std::vector<String> AppUtilWindows::getKeyboardLayoutList()
{
    std::vector<String> layoutLangCodes;
    {
        ...
        for (int i = 0; i < uLayouts; ++i) {
            String code("", 2);
            GetLocaleInfoA(MAKELCID(((ULONG_PTR)lpList[i] & 0xffffffff), SORT_DEFAULT), LOCALE_SISO639LANGNAME, &code[0], static_cast<int>(code.size()));
...
void AppUtilWindows::showNotification(const String &title, const String &text) const
{
    ...
    if (!WinToastLib::WinToast::instance()->isInitialized()) {
        ...
        const auto aumi = WinToastLib::WinToast::configureAUMI(L"" DESKFLOW_AUTHOR_NAME, L"" DESKFLOW_APP_NAME, L"" DESKFLOW_APP_NAME, L"1.14.1+");
