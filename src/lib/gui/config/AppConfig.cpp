...
void AppConfig::recallFromCurrentScope()
{
    ...
    m_MainWindowPosition = getFromCurrentScope<QPoint>(kMainWindowPosition, [](const QVariant &v) {
        return v.toPoint();
    });
    m_MainWindowSize = getFromCurrentScope<QSize>(kMainWindowSize, [](const QVariant &v) {
        return v.toSize();
    });
    ...
    m_EnableUpdateCheck = getFromCurrentScope<bool>(kEnableUpdateCheck, [](const QVariant &v) {
        return v.toBool();
    });
}
...
void AppConfig::determineScope()
{
    ...
    if (m_Scopes.scopeContains(settingName(Setting::kLoadSystemSettings), ConfigScopes::Scope::User)) {
        auto loadFromSystemScope = m_Scopes.getFromScope(settingName(Setting::kLoadSystemSettings), m_LoadFromSystemScope, ConfigScopes::Scope::User).toBool();
...
template<typename T>
void AppConfig::setInCurrentScope(Setting name, T value)
{
    m_Scopes.setInScope(settingName(name), value);
}

template<typename T>
void AppConfig::saveToAllScopes(Setting name, T value)
{
