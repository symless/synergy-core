    ...
    , m_pDeps(deps)
{
    ...
    connect(&m_pDeps->process(), &QProcessProxy::readyReadStandardOutput, this, &CoreProcess::onProcessReadyReadStandardOutput);

    connect(&m_pDeps->process(), &QProcessProxy::readyReadStandardError, this, &CoreProcess::onProcessReadyReadStandardError);
...
void CoreProcess::setProcessState(ProcessState state)
{
    ...
    qDebug("core process state changed: %s -> %s", //
