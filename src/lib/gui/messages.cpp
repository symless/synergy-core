...
void showErrorDialog(const QString &message, const QString &fileLine, QtMsgType type)
{
    ...
    if (type == QtFatalMsg) {
        text =
            "<p>Sorry, a fatal error has occurred and the application must "
            "now exit.</p>";
    } else {
    ...
    if (type == QtFatalMsg) {
        ...
        Errors::s_criticalMessage = std::make_unique<QMessageBox>(QMessageBox::Critical, title, text, QMessageBox::Ok | QMessageBox::Ignore);

        Errors::s_criticalMessage->open();

        QAction::connect(Errors::s_criticalMessage.get(),
