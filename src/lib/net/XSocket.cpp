...
XSocketAddress::XSocketAddress(EError error, const String &hostname, int port) _NOEXCEPT : m_error(error), m_hostname(hostname), m_port(port)
{
    // do nothing
}
...
String XSocketAddress::getWhat() const throw()
{
    ...
    return format(s_errorID[m_error], s_errorMsg[m_error], m_hostname.c_str(), deskflow::string::sprintf("%d", m_port).c_str());
}
