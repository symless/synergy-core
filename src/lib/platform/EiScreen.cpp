    ...
    , is_on_screen_(is_primary)
{
    ...
    events_->adoptHandler(Event::kSystem, events_->getSystemTarget(), new TMethodEventJob<EiScreen>(this, &EiScreen::handleSystemEvent));

    if (use_portal) {
        events_->adoptHandler(events_->forEi().connected(), getEventTarget(), new TMethodEventJob<EiScreen>(this, &EiScreen::handle_connected_to_eis_event));
        if (is_primary) {
...
#if WINAPI_LIBPORTAL
            events_->adoptHandler(events_->forEi().sessionClosed(),
                                  getEventTarget(),
...
void EiScreen::add_device(struct ei_device *device)
{
    ...
    if (!ei_pointer_ && ei_device_has_capability(device, EI_DEVICE_CAP_POINTER) && ei_device_has_capability(device, EI_DEVICE_CAP_BUTTON)
    ...
    if (!ei_abs_ && ei_device_has_capability(device, EI_DEVICE_CAP_POINTER_ABSOLUTE) && ei_device_has_capability(device, EI_DEVICE_CAP_BUTTON)
        && ei_device_has_capability(device, EI_DEVICE_CAP_SCROLL)) {
...
void EiScreen::on_pointer_scroll_event(ei_event *event)
{
    ...
    if (x != 0 || y != 0)
        sendEvent(events_->forIPrimaryScreen().wheel(), WheelInfo::alloc((int32_t)-x * PIXEL_TO_WHEEL_RATIO, (int32_t)-y * PIXEL_TO_WHEEL_RATIO));
    ...
    while ((event = ei_get_event(ei_)) != nullptr) {
        ...
        case EI_EVENT_SEAT_ADDED:
            if (!ei_seat_) {
                ...
                ei_seat_bind_capabilities(ei_seat_,
