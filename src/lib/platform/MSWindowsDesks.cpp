...
MSWindowsDesks::MSWindowsDesks(bool isPrimary, bool noHooks, const IScreenSaver *screensaver, IEventQueue *events, IJob *updateKeys, bool stopOnDeskSwitch)
...
void MSWindowsDesks::enable()
{
    ...
    m_events->adoptHandler(Event::kTimer, m_timer, new TMethodEventJob<MSWindowsDesks>(this, &MSWindowsDesks::handleCheckDesk));
...
void MSWindowsDesks::setShape(SInt32 x, SInt32 y, SInt32 width, SInt32 height, SInt32 xCenter, SInt32 yCenter, bool isMultimon)
{
    m_x = x;
    m_y = y;
    m_w = width;
...
void MSWindowsDesks::deskMouseMove(SInt32 x, SInt32 y) const
{
    ...
    send_mouse_input(MOUSEEVENTF_MOVE | MOUSEEVENTF_ABSOLUTE, (DWORD)((65535.0f * x) / (w - 1) + 0.5f), (DWORD)((65535.0f * y) / (h - 1) + 0.5f), 0);
}
...
void MSWindowsDesks::deskMouseRelativeMove(SInt32 dx, SInt32 dy) const
{
    ...
    int oldSpeed[4];
    bool accelChanged = SystemParametersInfo(SPI_GETMOUSE, 0, oldSpeed, 0) && SystemParametersInfo(SPI_GETMOUSESPEED, 0, oldSpeed + 3, 0);

    // use 1:1 motion
    if (accelChanged) {
        int newSpeed[4] = {0, 0, 0, 1};
        accelChanged = SystemParametersInfo(SPI_SETMOUSE, 0, newSpeed, 0) || SystemParametersInfo(SPI_SETMOUSESPEED, 0, newSpeed + 3, 0);
    }
...
void MSWindowsDesks::deskThread(void *vdesk)
{
    ...
    while (GetMessage(&msg, NULL, 0, 0)) {
        ...
        case DESKFLOW_MSG_FAKE_INPUT:
            send_keyboard_input(DESKFLOW_HOOK_FAKE_INPUT_VIRTUAL_KEY, DESKFLOW_HOOK_FAKE_INPUT_SCANCODE, msg.wParam ? 0 : KEYEVENTF_KEYUP);
            break;
        }
