...
MSWindowsKeyState::MSWindowsKeyState(MSWindowsDesks *desks, void *eventTarget, IEventQueue *events, std::vector<String> layouts, bool isLangSyncEnabled)
...
void MSWindowsKeyState::sendKeyEvent(void *target, bool press, bool isAutoRepeat, KeyID key, KeyModifierMask mask, SInt32 count, KeyButton button)
{
...
bool MSWindowsKeyState::fakeKeyRepeat(KeyID id, KeyModifierMask mask, SInt32 count, KeyButton button, const String &lang)
{
        ...
        for (KeyButton i = 0; i < 512; ++i) {
            if (m_buttonToVK[i] != 0) {
                ...
                if (item.m_id == 0) {
                    ...
                    static const Modifier modifiers[] = {{VK_SHIFT, VK_SHIFT, 0x80u, KeyModifierShift},
...
KeyID MSWindowsKeyState::getIDForKey(deskflow::KeyMap::KeyItem &item, KeyButton button, UINT virtualKey, PBYTE keyState, HKL hkl) const
{
    WCHAR unicode[2];
