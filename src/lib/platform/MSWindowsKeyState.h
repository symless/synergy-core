...
{
public:
    MSWindowsKeyState(MSWindowsDesks *desks, void *eventTarget, IEventQueue *events, std::vector<String> layouts, bool isLangSyncEnabled);
    MSWindowsKeyState(MSWindowsDesks *desks,
                      void *eventTarget,
                      IEventQueue *events,
...
private:
    ...
    typedef int(WINAPI *ToUnicodeEx_t)(UINT wVirtKey, UINT wScanCode, PBYTE lpKeyState, LPWSTR pwszBuff, int cchBuff, UINT wFlags, HKL dwhkl);
