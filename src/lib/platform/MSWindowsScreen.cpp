    ...
    , m_dropWindowSize(20)
{
    ...
    try {
        ...
        m_desks = new MSWindowsDesks(m_isPrimary,
                                     ...
                                     stopOnDeskSwitch);
        m_keyState = new MSWindowsKeyState(m_desks, getEventTarget(), m_events, AppUtil::instance().getKeyboardLayoutList(), enableLangSync);
    ...
    m_events->adoptHandler(Event::kSystem, m_events->getSystemTarget(), new TMethodEventJob<MSWindowsScreen>(this, &MSWindowsScreen::handleSystemEvent));
...
void MSWindowsScreen::enable()
{
    ...
    m_events->adoptHandler(Event::kTimer, m_fixTimer, new TMethodEventJob<MSWindowsScreen>(this, &MSWindowsScreen::handleFixes));
...
{
    static const char *buttonToName[] = {"<invalid>", "Left Button", "Middle Button", "Right Button", "X Button 1", "X Button 2"};
...
bool MSWindowsScreen::onKey(WPARAM wParam, LPARAM lParam)
{
    ...
    if (!ignore()) {
        ...
        if (key != kKeyNone) {
            // do it
            m_keyState
                ->sendKeyEvent(getEventTarget(), ((lParam & 0x80000000u) == 0), ((lParam & 0x40000000u) != 0), key, mask, (SInt32)(lParam & 0xffff), button);
        } else {
...
bool MSWindowsScreen::onMouseMove(SInt32 mx, SInt32 my)
{
    ...
    if (m_isOnScreen) {
        ...
        if (-x + bogusZoneSize > m_xCenter - m_x || x + bogusZoneSize > m_x + m_w - m_xCenter || -y + bogusZoneSize > m_yCenter - m_y
