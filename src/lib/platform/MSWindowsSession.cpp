    ...
    while (gotEntry) {
        ...
        if (entry.th32ProcessID != 0) {
            ...
            if (!pidToSidRet) {
                ...
                // simply ignore it
                LOG((CLOG_DEBUG2 "could not get session id for process: %i %s, code=%i", entry.th32ProcessID, entry.szExeFile, GetLastError()));
...
MSWindowsSession::getUserToken(LPSECURITY_ATTRIBUTES security)
{
    ...
    HANDLE newToken;
    if (!DuplicateTokenEx(sourceToken, TOKEN_ASSIGN_PRIMARY | TOKEN_ALL_ACCESS, security, SecurityImpersonation, TokenPrimary, &newToken)) {
