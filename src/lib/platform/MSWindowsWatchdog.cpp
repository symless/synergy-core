...
MSWindowsWatchdog::MSWindowsWatchdog(bool autoDetectCommand, IpcServer &ipcServer, IpcLogOutputter &ipcLogOutputter, bool foreground)
    : m_thread(NULL)
...
MSWindowsWatchdog::duplicateProcessToken(HANDLE process, LPSECURITY_ATTRIBUTES security)
{
    ...
    HANDLE newToken;
    BOOL duplicateRet = DuplicateTokenEx(sourceToken, TOKEN_ASSIGN_PRIMARY | TOKEN_ALL_ACCESS, security, SecurityImpersonation, TokenPrimary, &newToken);
...
BOOL MSWindowsWatchdog::startProcessAsUser(String &command, HANDLE userToken, LPSECURITY_ATTRIBUTES sa)
{
    ...
    BOOL createRet = CreateProcessAsUser(userToken, NULL, LPSTR(command.c_str()), sa, NULL, TRUE, creationFlags, environment, NULL, &si, &m_processInfo);
...
void MSWindowsWatchdog::shutdownExistingProcesses()
{
    ...
    while (gotEntry) {
        ...
        if (entry.th32ProcessID != 0) {
            if (_stricmp(entry.szExeFile, CLIENT_BINARY_NAME ".exe") == 0 || _stricmp(entry.szExeFile, SERVER_BINARY_NAME ".exe") == 0
