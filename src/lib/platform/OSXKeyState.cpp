...
OSXKeyState::OSXKeyState(IEventQueue *events, deskflow::KeyMap &keyMap, std::vector<String> layouts, bool isLangSyncEnabled)
...
void OSXKeyState::fakeKey(const Keystroke &keystroke)
{
    ...
    case Keystroke::kButton: {
        ...
        LOG((CLOG_DEBUG1 "  button=0x%04x virtualKey=0x%04x keyDown=%s client=0x%04x", button, virtualKey, keyDown ? "down" : "up", client));
...
bool OSXKeyState::mapDeskflowHotKeyToMac(KeyID key, KeyModifierMask mask, UInt32 &macVirtualKey, UInt32 &macModifierMask) const
{
