...
void PortalInputCapture::cb_init_input_capture_session(GObject *object, GAsyncResult *res)
{
    ...
    signals_[ZONES_CHANGED] = g_signal_connect(G_OBJECT(session_), "zones-changed", G_CALLBACK(cb_zones_changed_cb), this);
    XdpSession *parent_session = xdp_input_capture_session_get_session(session);
    signals_[SESSION_CLOSED] = g_signal_connect(G_OBJECT(parent_session), "closed", G_CALLBACK(cb_session_closed_cb), this);
...
void PortalInputCapture::cb_zones_changed(XdpInputCaptureSession *session, GVariant *options)
{
    ...
    while (zones != nullptr) {
        ...
        barriers_.push_back(
            XDP_INPUT_CAPTURE_POINTER_BARRIER(g_object_new(XDP_TYPE_INPUT_CAPTURE_POINTER_BARRIER, "id", id, "x1", x1, "y1", y1, "x2", x2, "y2", y2, nullptr)));
        id = barriers_.size() + 1;
        x1 = x + w;
        y1 = y;
        x2 = x + w;
