...
void XWindowsClipboard::addRequest(Window owner, Window requestor, Atom target, ::Time time, Atom property)
{
    ...
    bool success = false;
    if (owner == m_window) {
        LOG((CLOG_DEBUG1 "request for clipboard %d, target %s by 0x%08x (property=%s)",
             m_selection,
...
bool XWindowsClipboard::icccmGetSelection(Atom target, Atom *actualTarget, String *data) const
{
    ...
    if (CICCCMGetClipboard getter(m_window, m_time, m_atomData); !getter.readClipboard(m_display, m_selection, target, actualTarget, data)) {
...
void XWindowsClipboard::motifFillCache()
{
    ...
    for (SInt32 i = 0; i < numFormats; ++i) {
        ...
        if (motifFormat.m_id != MotifClip::Format || motifFormat.m_length < 0 || motifFormat.m_type == None || motifFormat.m_deleted != 0) {
            continue;
        }
...
bool XWindowsClipboard::sendReply(Reply *reply)
{
    ...
    if (!failed) {
        ...
        if (useINCR && !reply->m_replied) {
            ...
            if (!XWindowsUtil::setWindowProperty(m_display, reply->m_requestor, reply->m_property, &size, 4, m_atomINCR, 32)) {
                failed = true;
            }
        }
    ...
    if (failed) {
        ...
        if (!reply->m_replied) {
            ...
            XWindowsUtil::setWindowProperty(m_display, reply->m_requestor, reply->m_property, &dummy, 0, reply->m_type, reply->m_format);
    ...
    if (CLOG->getFilter() < kDEBUG2) {
        sendNotify(reply->m_requestor, m_selection, reply->m_target, reply->m_property, static_cast<unsigned int>(reply->m_time));
...
bool XWindowsClipboard::CICCCMGetClipboard::readClipboard(Display *display, Atom selection, Atom target, Atom *actualTarget, String *data)
{
    assert(actualTarget != nullptr);
    assert(data != nullptr);
...
XWindowsClipboard::Reply::Reply(Window requestor, Atom target, ::Time time, Atom property, const String &data, Atom type, int format)
    : m_requestor(requestor)
    , m_target(target)
    , m_time(time)
    , m_property(property)
