...
{
    return static_cast<SInt32>(static_cast<UInt32>(data[0]) | (static_cast<UInt32>(data[1]) << 8) | (static_cast<UInt32>(data[2]) << 16)
...
String XWindowsClipboardAnyBitmapConverter::fromIClipboard(const String &bmp) const
{
    ...
    if (infoHeader.biSize != 40 || infoHeader.biWidth == 0 || infoHeader.biHeight == 0 || infoHeader.biPlanes != 0 || infoHeader.biCompression != 0
