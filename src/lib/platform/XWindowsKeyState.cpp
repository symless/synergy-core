...
bool XWindowsKeyState::setCurrentLanguageWithDBus(SInt32 group) const
{
    ...
    QString method = "Eval";
    QString param = "imports.ui.status.keyboard.getInputSourceManager().inputSources[" + QString::number(group) + "].activate()";
    ...
    if (!reply.isValid()) {
        ...
        LOG((CLOG_WARN "keyboard layout fail %s : %s", qerror.name().toStdString().c_str(), qerror.message().toStdString().c_str()));
        return true;
    }
...
void XWindowsKeyState::fakeKey(const Keystroke &keystroke)
{
    ...
    case Keystroke::kButton:
        if (keystroke.m_data.m_button.m_repeat) {
            ...
            int i = (c >> 3);
            int b = 1 << (c & 7);
            if (m_keyboardState.global_auto_repeat == AutoRepeatModeOff || (c != 113 && c != 116 && (m_keyboardState.auto_repeats[i] & b) == 0)) {
                LOG((CLOG_DEBUG1 "  discard autorepeat"));
                break;
            }
        }
        XTestFakeKeyEvent(m_display, keystroke.m_data.m_button.m_button, keystroke.m_data.m_button.m_press ? True : False, CurrentTime);
        break;
...
#if HAVE_XKB_EXTENSION
            if (m_xkb != NULL) {
                if (XkbLockGroup(m_display, XkbUseCoreKbd, getEffectiveGroup(pollActiveGroup(), keystroke.m_data.m_group.m_group)) == False) {
                    LOG((CLOG_DEBUG1 "xkb lock group request not sent"));
                }
            } else
#endif
...
void XWindowsKeyState::updateKeysymMap(deskflow::KeyMap &keyMap)
{
    ...
    for (int i = 0; i < numKeycodes; ++i) {
        ...
        if (IsKeypadKey(keysyms[0]) || IsPrivateKeypadKey(keysyms[0]) || IsKeypadKey(keysyms[1]) || IsPrivateKeypadKey(keysyms[1]) || IsKeypadKey(keysyms[2])
            || IsPrivateKeypadKey(keysyms[2]) || IsKeypadKey(keysyms[3]) || IsPrivateKeypadKey(keysyms[3])) {
    ...
    for (int i = m_xkb->min_key_code; i <= m_xkb->max_key_code; ++i) {
        ...
        for (int group = 0; group < maxNumGroups; ++group) {
            ...
            for (int j = -1; j < type->map_count; ++j) {
                ...
                if ((item.m_required & LockMask) != 0 && j != -1 && type->preserve != NULL && (type->preserve[j].mask & LockMask) != 0) {
