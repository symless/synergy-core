...
XWindowsScreen::XWindowsScreen(const char *displayName,
                               bool isPrimary,
    ...
    , m_events(events)
{
    ...
    m_events->adoptHandler(Event::kSystem, m_events->getSystemTarget(), new TMethodEventJob<XWindowsScreen>(this, &XWindowsScreen::handleSystemEvent));
...
void XWindowsScreen::warpCursor(SInt32 x, SInt32 y)
{
    ...
    XEvent event;
    while (XCheckMaskEvent(m_display, PointerMotionMask | ButtonPressMask | ButtonReleaseMask | KeyPressMask | KeyReleaseMask | KeymapStateMask, &event)) {
        // do nothing
    }
...
UInt32 XWindowsScreen::registerHotKey(KeyID key, KeyModifierMask mask)
{
    ...
    bool err = false;
    {
        ...
        if (key == kKeyNone) {
            static const KeyModifierMask s_hotKeyModifiers[] = {KeyModifierShift, KeyModifierControl, KeyModifierAlt, KeyModifierMeta, KeyModifierSuper};
...
Window XWindowsScreen::openWindow() const
{
    ...
    if (m_isPrimary) {
        ...
        // the whole screen.
        attr.event_mask = PointerMotionMask | ButtonPressMask | ButtonReleaseMask | KeyPressMask | KeyReleaseMask | KeymapStateMask | PropertyChangeMask;
        x = m_x;
        y = m_y;
        w = m_w;
        h = m_h;
    ...
    Window window =
        XCreateWindow(m_display, m_root, x, y, w, h, 0, 0, InputOnly, CopyFromParent, CWDontPropagate | CWEventMask | CWOverrideRedirect | CWCursor, &attr);
    if (window == None) {
        throw XScreenOpenFailure();
    }
    return window;
...
Bool XWindowsScreen::findKeyEvent(Display *, XEvent *xevent, XPointer arg)
{
    ...
    return (xevent->type == filter->m_event && xevent->xkey.window == filter->m_window && xevent->xkey.time == filter->m_time
...
void XWindowsScreen::handleSystemEvent(const Event &event, void *)
{
    ...
    if (m_xi2detected) {
        ...
        if (XGetEventData(m_display, cookie) && cookie->type == GenericEvent && cookie->extension == xi_opcode) {
            if (cookie->evtype == XI_RawMotion) {
                ...
                unsigned int msk;
                xmotion.same_screen =
                    XQueryPointer(m_display, m_root, &xmotion.root, &xmotion.subwindow, &xmotion.x_root, &xmotion.y_root, &xmotion.x, &xmotion.y, &msk);
                onMouseMove(xmotion);
    ...
    case SelectionRequest: {
        ...
        if (id != kClipboardEnd) {
            m_clipboard[id]->addRequest(xevent->xselectionrequest.owner,
    ...
    default:
        ...
        if (m_xrandr) {
            if (xevent->type == m_xrandrEventBase + RRScreenChangeNotify
                || xevent->type == m_xrandrEventBase + RRNotify && reinterpret_cast<XRRNotifyEvent *>(xevent)->subtype == RRNotify_CrtcChange) {
...
void XWindowsScreen::onKeyPress(XKeyEvent &xkey)
{
    ...
    if (key != kKeyNone) {
        ...
        if ((key == kKeyPause || key == kKeyBreak) && (mask & (KeyModifierControl | KeyModifierAlt)) == (KeyModifierControl | KeyModifierAlt)) {
...
void XWindowsScreen::onMouseMove(const XMotionEvent &xmotion)
{
    ...
    if (xmotion.send_event) {
        ...
        if (xmotion.x_root - m_xCenter < -s_size || xmotion.x_root - m_xCenter > s_size || xmotion.y_root - m_yCenter < -s_size
...
bool XWindowsScreen::grabMouseAndKeyboard()
{
    ...
    do {
        ...
        result = XGrabPointer(m_display, m_window, False, event_mask, GrabModeAsync, GrabModeAsync, m_window, None, CurrentTime);
        assert(result != GrabNotViewable);
        if (result != GrabSuccess) {
