...
bool XWindowsUtil::getWindowProperty(Display *display, Window window, Atom property, String *data, Atom *type, SInt32 *format, bool deleteProperty)
{
    assert(display != NULL);
...
bool XWindowsUtil::setWindowProperty(Display *display, Window window, Atom property, const void *vdata, UInt32 size, Atom type, SInt32 format)
{
...
Bool XWindowsUtil::propertyNotifyPredicate(Display *, XEvent *xevent, XPointer arg)
{
    ...
    return (xevent->type == PropertyNotify && xevent->xproperty.window == filter->m_window && xevent->xproperty.atom == filter->m_property
...
XWindowsUtil::ErrorLock::ErrorLock(Display *display, ErrorHandler handler, void *data)
    : m_display(display)
{
