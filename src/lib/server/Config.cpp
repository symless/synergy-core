...
bool Config::connect(const String &srcName, EDirection srcSide, float srcStart, float srcEnd, const String &dstName, float dstStart, float dstEnd)
{
...
void Config::readSectionLinks(ConfigReadContext &s)
{
    ...
    while (s.readLine(line)) {
        ...
        if (line[line.size() - 1] == ':') {
            ...
            if (!connect(screen, dir, srcInterval.first, srcInterval.second, dstScreen, dstInterval.first, dstInterval.second)) {
...
InputFilter::Condition *Config::parseCondition(ConfigReadContext &s, const String &name, const std::vector<String> &args)
{
    if (name == "keystroke") {
        if (args.size() != 1) {
...
void Config::parseAction(ConfigReadContext &s, const String &name, const std::vector<String> &args, InputFilter::Rule &rule, bool activate)
{
...
{
    if (id == kOptionHalfDuplexCapsLock || id == kOptionHalfDuplexNumLock || id == kOptionHalfDuplexScrollLock || id == kOptionScreenSwitchNeedsShift
        || id == kOptionScreenSwitchNeedsControl || id == kOptionScreenSwitchNeedsAlt || id == kOptionXTestXineramaUnaware || id == kOptionRelativeMouseMoves
        || id == kOptionWin32KeepForeground || id == kOptionScreenPreserveFocus || id == kOptionClipboardSharing || id == kOptionClipboardSharingSize) {
        return (value != 0) ? "true" : "false";
    }
    if (id == kOptionModifierMapForShift || id == kOptionModifierMapForControl || id == kOptionModifierMapForAlt || id == kOptionModifierMapForAltGr
    ...
    if (id == kOptionHeartbeat || id == kOptionScreenSwitchCornerSize || id == kOptionScreenSwitchDelay || id == kOptionScreenSwitchTwoTap) {
...
bool Config::CellEdge::overlaps(const CellEdge &edge) const
{
    ...
    return (x.first >= y.first && x.first < y.second) || (x.second > y.first && x.second <= y.second) || (y.first >= x.first && y.first < x.second)
...
std::ostream &operator<<(std::ostream &s, const Config &config)
{
    ...
    for (Config::const_iterator screen = config.begin(); screen != config.end(); ++screen) {
        ...
        for (Config::link_const_iterator link = config.beginNeighbor(*screen), nend = config.endNeighbor(*screen); link != nend; ++link) {
    ...
    if (config.m_map.size() != config.m_nameToCanonicalName.size()) {
        ...
        CMNameMap aliases;
        for (Config::NameMap::const_iterator index = config.m_nameToCanonicalName.begin(); index != config.m_nameToCanonicalName.end(); ++index) {
...
void ConfigReadContext::parseNameWithArgs(const String &type, const String &line, const String &delim, String::size_type &index, String &name, ArgList &args)
    const
{
...
IPlatformScreen::KeyInfo *ConfigReadContext::parseKeystroke(const String &keystroke, const std::set<String> &screens) const
{
    String s = keystroke;
...
XConfigRead::XConfigRead(const ConfigReadContext &context, const char *errorFmt, const String &arg)
    : m_error(deskflow::string::sprintf("line %d: ", context.getLineNumber()) + deskflow::string::format(errorFmt, arg.c_str()))
{
    // do nothing
}
