...
{
    static const KeyModifierMask s_ignoreMask = KeyModifierAltGr | KeyModifierCapsLock | KeyModifierNumLock | KeyModifierScrollLock;
...
{
    static const Server::LockCursorToScreenInfo::State s_state[] = {Server::LockCursorToScreenInfo::kOff,
    ...
    m_events->addEvent(Event(m_events->forServer().lockCursorToScreen(), event.getTarget(), info, Event::kDeliverImmediately));
}
...
void InputFilter::SwitchInDirectionAction::perform(const Event &event)
{
    ...
    m_events->addEvent(Event(m_events->forServer().switchInDirection(), event.getTarget(), info, Event::kDeliverImmediately));
}
...
InputFilter::KeyboardBroadcastAction::KeyboardBroadcastAction(IEventQueue *events, Mode mode, const std::set<String> &screens)
    : m_mode(mode)
...
String InputFilter::KeyboardBroadcastAction::format() const
{
    ...
    if (m_screens.empty() || m_screens[0] == '*') {
        ...
        return deskflow::string::sprintf("%s(%s,%.*s)",
                                         s_name,
                                         s_mode[m_mode],
...
{
    static const Server::KeyboardBroadcastInfo::State s_state[] = {Server::KeyboardBroadcastInfo::kOff,
    ...
    m_events->addEvent(Event(m_events->forServer().keyboardBroadcast(), event.getTarget(), info, Event::kDeliverImmediately));
}
...
String InputFilter::KeystrokeAction::format() const
{
    ...
    if (m_keyInfo->m_screens[0] == '\0') {
        return deskflow::string::sprintf("%s(%s)", type, deskflow::KeyMap::formatKey(m_keyInfo->m_key, m_keyInfo->m_mask).c_str());
...
void InputFilter::KeystrokeAction::perform(const Event &event)
{
    ...
    m_events->addEvent(Event(m_events->forIPrimaryScreen().fakeInputBegin(), event.getTarget(), NULL, Event::kDeliverImmediately));
    m_events->addEvent(Event(type, event.getTarget(), m_keyInfo, Event::kDeliverImmediately | Event::kDontFreeData));
    m_events->addEvent(Event(m_events->forIPrimaryScreen().fakeInputEnd(), event.getTarget(), NULL, Event::kDeliverImmediately));
}
...

{
    ...
    if (m_buttonInfo->m_mask != 0) {
        ...
        m_events->addEvent(Event(m_events->forIKeyState().keyDown(), event.getTarget(), modifierInfo, Event::kDeliverImmediately));
    }
...
void InputFilter::handleEvent(const Event &event, void *)
{
    ...
    Event myEvent(event.getType(), this, event.getData(), event.getFlags() | Event::kDontFreeData | Event::kDeliverImmediately);
